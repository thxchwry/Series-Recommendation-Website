...
export default function MyCard({ name, picture, id, onDelete }) {
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/insert/europe/${id}`, {
        method: 'DELETE',
      })
    ...
    <Card sx={{ maxWidth: 250 }}>
      ...
      <CardContent sx={{ textAlign: 'left' }}>
        ...
        <Stack direction="row" spacing={1}>
          <Button variant="outlined" size="small" color="primary" href={`/admin/europe/update/${id}`}>
            Edit
          </Button>
          <Button variant="outlined" size="small" color="error" onClick={handleDelete} href={`/admin/europe`}>
            Delete
          </Button>
        </Stack>
      </CardContent>
