...
export async function GET(request, { params }) {
  ...
  try {
    const [rows] = await db.query("SELECT * FROM serieschinese WHERE id = ?", [id]);
...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    const [checkExist] = await db.query("SELECT id FROM serieschinese WHERE id = ?", [id]);
    if (checkExist.length === 0) {
      return NextResponse.json({ error: "Series not found" }, { status: 404 });
    }

    const [result] = await db.query("DELETE FROM serieschinese WHERE id = ?", [id]);
