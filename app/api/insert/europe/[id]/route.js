...
export async function GET(request, { params }) {
  ...
  try {
    const [rows] = await db.query("SELECT * FROM serieseurope WHERE id = ?", [id]);
...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    const [checkExist] = await db.query("SELECT id FROM serieseurope WHERE id = ?", [id]);
    if (checkExist.length === 0) {
      return NextResponse.json({ error: "Series not found" }, { status: 404 });
    }

    // ลบข้อมูล
    const [result] = await db.query("DELETE FROM serieseurope WHERE id = ?", [id]);
